...
<script>
	function checkUserIdDuplicate(el) {
		...
		$.get('../member/getLoginIdDup', {
			...
			if (data.success) {
				$('.checkEmptyMsg').html('<div class="mt-2 text-green-500">' + data.msg + '</div>')
				validLoginId = data.data1;
			} else {
				$('.checkEmptyMsg').html('<div class="mt-2 text-red-500">' + data.msg + '</div>')
				validLoginId = '';
			}
		}, 'json');
	...
	function checkUserIdEmpty(el) {
		...
		if (value.length == 0) {
			console.log("실행됨2");
			$('.checkEmptyMsg').html('<div class="mt-2 text-red-500">' + msg + '가 입력되지 않았습니다. </div>')
		}
...
<div
	...
	<div
		...
		<div
			...
			<p
				class="mt-2 flex-grow-0 flex-shrink-0 w-[510px] h-[50px] text-5xl text-center text-black">회원가입</p>
		</div>
		<form action="doJoin" id="joinForm" method="POST">
			<div
				...
				<div
					class="flex-grow-0 flex-shrink-0 w-[380px] h-[90px] relative overflow-hidden">
					<p
						class="w-[78px] h-5 absolute left-[13px] top-[3px] text-lg text-left text-black">이메일</p>
					<div
				...
				<div
					class="flex-grow-0 flex-shrink-0 w-[380px] h-[90px] relative overflow-hidden">
					<p
						class="w-[78px] h-5 absolute left-[13px] top-[3px] text-lg text-left text-black">이름</p>
					<div
						class="w-[341px] h-[46px] absolute left-[13px] top-7 bg-[#f4f5f5] border border-[#757678]"></div>
					<input
						class="w-[289px] h-[46px] absolute left-[65px] top-7 bg-[#f4f5f5] border border-[#757678] pl-5 text-lg"
		...
		<div
			...
			<button form="joinForm"
				class="w-full flex-grow-0 flex-shrink-0 text-2xl text-center text-white cursor-pointer">회원가입</button>
		</div>
