window.onload = function() {
	kakao.maps.load(function() {
		...
		// 영역 색깔
		// 지자체 로고, 관광 브랜드, 공공시설 컬러를 바탕으로 정리된 것
		const colors = [
			'#0078D4', // 서울특별시 (단청 청색 기반)
			'#009DDC', // 부산광역시 (바다 블루)
			'#DA291C', // 대구광역시 (팔공산 단풍색, 열정적 붉은색 계열)
			'#005BAC', // 인천광역시 (항만·하늘·바다 CI 블루)
			'#00A94F', // 광주광역시 (빛고을 → 녹색 빛 이미지)
			'#C8102E', // 대전광역시 (엑스포 붉은색 계열, 에너지 상징)
			'#007A33', // 울산광역시 (산업+생태 조화, 녹색 계열)
			'#5B3A91', // 세종특별자치시 (행정중심복합도시 → 보라 계열 지성 이미지)
			'#F58220', // 경기도 (역동성과 다양성 → 따뜻한 주황)
			'#009473', // 강원도 (산림, 자연 → 청록)
			'#66BC29', // 충청북도 (자연·유기농 상징 초록)
			'#1C7C54', // 충청남도 (내포 문화권, 유교적 전통 초록 계열)
			'#D94F30', // 전라북도 (농업+전통문화 → 따뜻한 붉은 계열)
			'#008272', // 전라남도 (남도 바다와 섬 이미지, 진한 청록)
			'#002F6C', // 경상북도 (유교문화, 청렴 이미지 → 네이비 블루)
			'#D82E3F', // 경상남도 (남해, 활기찬 도시 → 붉은 계열)
			'#FBB13C'  // 제주특별자치도 (귤, 화산, 따뜻한 태양 오렌지)
		];
